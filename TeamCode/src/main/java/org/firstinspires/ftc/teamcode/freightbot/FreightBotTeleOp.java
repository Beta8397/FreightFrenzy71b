...
public class FreightBotTeleOp extends MecBotTeleOp {
    ...
    private float tapeElevationPosition = 0.5f * (FreightBot.TAPE_ELEVATION_MAX + FreightBot.TAPE_ELEVATION_MIN);
    ...
    FreightBot bot = new FreightBot(MecBot.MotorType.NeverestOrbital20, 13, 10, 6, 45, 1,
    ...
    public void runOpMode() {
        ...
        bot.setIntakeFlipper(FreightBot.INTAKE_FLIPPER_CENTER);
        bot.openArmCapServo();
        ...
        while(opModeIsActive()) {
            ...
            if (toggle2APressed) {
               ...
               armServoPosition = dumperExtended ? 0.98f - 0.000395f * rightArmAngleMotorPosition :
                       FreightBot.DUMPER_RETRACTED;
               bot.setArmServoPosition(armServoPosition);
            } else if (gamepad2.back) {
                dumperExtended = false;
                armServoPosition = FreightBot.DUMPER_RETRACTED;
                bot.setArmServoPosition(armServoPosition);
            }
...
//            }
            ...
            if (gamepad2.right_stick_y < -0.5) {
                ...
                if (actualArmExtensionTicks > 100) {
                    bot.setArmAngleTicks(FreightBot.MAX_ARM_ANGLE_TICKS);
                    armServoPosition = 0.98f - 0.000395f * rightArmAngleMotorPosition;
                    bot.setArmServoPosition(armServoPosition);
                }
            }
...
//            }
            ...
            switch (bot.getIntakeState()) {
                ...
                case CENTER_MIN:
                    if (toggle1BPressed) {
                        bot.setIntakeState(FreightBot.IntakeState.OUT_MAX);
                ...
                case CENTER_MID:
                    if (toggle1BPressed) {
                        ...
                        bot.setIntakeState(FreightBot.IntakeState.CENTER_MIN);
                    }
                    break;
            }
            ...
            switch (bot.getIntakeWheelState()) {
                case STOPPED:
                    if (toggleBothLeftBumper) {
                        bot.setIntakeWheelState(FreightBot.IntakeWheelState.REVERSE);
                    } else if (toggleRight1BumperPressed) {
                        bot.setIntakeWheelState(FreightBot.IntakeWheelState.FORWARD);
                    } else if (toggle1BPressed && bot.getIntakeState() == FreightBot.IntakeState.OUT_MAX) {
                        bot.setIntakeWheelState(FreightBot.IntakeWheelState.FORWARD);
                    }
                    break;
                case FORWARD:
                    ...
                    } else if (toggle1BPressed && bot.getIntakeState() == FreightBot.IntakeState.IN_MID) {
                        bot.setIntakeWheelState(FreightBot.IntakeWheelState.STOPPED);
                    }
                    break;
                case REVERSE:
...
//            }
            ...
            if (gamepad2.dpad_down){
                ...
                tapeElevationPosition = Range.clip(tapeElevationPosition, FreightBot.TAPE_ELEVATION_MIN,
                        FreightBot.TAPE_ELEVATION_MAX);
            } else if (gamepad2.dpad_up) {
                tapeElevationPosition -= 0.003f;
                tapeElevationPosition = Range.clip(tapeElevationPosition, FreightBot.TAPE_ELEVATION_MIN,
                        FreightBot.TAPE_ELEVATION_MAX);
